...
立教大学の公式シラバスシステムを逆引きして、より使いやすい検索 UI・CLI・AI 連携を提供するツールです。

> **注意**: 本ツールは立教大学公式のものではありません。上游サーバーへの過度なリクエストは避けてください。

## Features

- **Web UI** — Flask ベースの検索画面。曜日時限グリッド、評価方式フィルタ、シラバス詳細モーダル
- **CLI** — ターミナルから JSON で結果を取得。AI agent の subprocess 呼び出しに最適
- **MCP Server** — Model Context Protocol 対応。Claude / Cursor 等の AI から直接ツールとして呼び出し可能
- **自然言語検索** — `月曜2限の経済学部の英語` のような自由文から自動でパラメータ抽出
- **課程比較** — 複数科目のシラバスを並列取得して比較
